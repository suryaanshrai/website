    ...
    try {
        ...
        if (response.ok) {
            ...
            if (sourceElement) {
                ...
                const currentPlayingSrc = audioElement.currentSrc ? new URL(audioElement.currentSrc, window.location.origin).pathname : ""
                ...
                if (decodedCurrent !== decodedNew) {
                    console.log("Audio mismatch detected, reloading player:", { current: decodedCurrent, new: decodedNew })
